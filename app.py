# PROJEK DATA SAINS OBJECT DETECTION
...
with st.sidebar:
    ...
    # Mengatur judul sidebar "Panel Utama" rata tengah (Caption lama di sini sudah dihapus)
    st.markdown("""
    <div style="text-align: center;">
        <h2 style="margin-bottom: 0; font-family: sans-serif;">Panel Menu Utama</h2>
    </div>
    ...
    st.markdown("""
    <div style="text-align: center;">
        <p style="color: gray; font-size: 0.85rem; margin-top: 5px; margin-bottom: 0;">RoadCheck AI • 2026</p>
    </div>
    """, unsafe_allow_html=True)
...
if st.session_state.menu_aktif == "Main Program":
    # Judul RoadCheck dengan background merah
    st.markdown("""
    ...
    st.markdown("<h3 style='text-align: center;'>Sistem Pendeteksi Objek Otomatis Pada Kerusakan Jalan</h3>", unsafe_allow_html=True)
    st.write("---")
    
    st.markdown("### Masukan Data Lapangan")
    file_terunggah = st.file_uploader("Drag and drop file here", type=["jpg", "jpeg", "png"])
    
    # UPDATE BARU: Teks peringatan merah diletakkan tepat di bawah area drag and drop
    st.markdown("""
    <p style="color: #FF4B4B; font-weight: bold; margin-top: -10px; margin-bottom: 20px; font-size: 0.9rem;">
        Pastikan Anda memasukkan gambar jalanan dengan resolusi yang jelas agar model dapat mendeteksi kerusakan secara akurat!
    </p>
    """, unsafe_allow_html=True)
    
    # Jika user mengunggah foto baru, simpan ke memori permanen dan paksa konversi ke RGB (3 channel)
    if file_terunggah is not None:
        img_raw = Image.open(file_terunggah)
        st.session_state.foto_saved = img_raw.convert('RGB')
        st.session_state.sudah_deteksi = False  # Mereset hasil deteksi lama jika ganti foto baru

    # Jika di dalam memori ada foto yang tersimpan, tampilkan ke layar
    if st.session_state.foto_saved is not None:
        st.image(st.session_state.foto_saved, caption="Pratinjau Gambar", use_container_width=True)
        
        tombol_deteksi = st.button("Deteksi !", type="primary")
        st.write("---")
        
        if tombol_deteksi:
            with st.spinner("Model sedang menganalisis objek kerusakan..."):
                try:
                    ...
                    if len(st.session_state.list_kerusakan) > 0:
                        ...
                        st.session_state.penjelasan_singkat = "Sistem model tidak mendeteksi adanya indikasi kerusakan struktural maupun fungsional pada permukaan sampel gambar ini."
                    
                    st.session_state.sudah_deteksi = True
                
                except Exception as e:
                    st.error(f"Terjadi kesalahan teknis saat memproses gambar: {str(e)}")
                    st.info("Saran: Pastikan berkas yang diunggah adalah file gambar valid yang tidak rusak (corrupted).")

        # BLOK UTAMA ANTI-RESET
        if st.session_state.sudah_deteksi:
            st.markdown("### Hasil Pendeteksian Objek")
            st.image(st.session_state.gambar_hasil, channels="RGB", caption="Visualisasi Bounding Box", use_container_width=True)
            ...
            if st.session_state.status_level != "Aman":
                ...
                if st.session_state.status_level == "Low":
                    ...
                    *Aksi Perbaikan:* Bersihkan celah-celah retakan garis menggunakan udara bertekanan kompresor, kemudian injeksikan bahan pengisi berupa aspal cair panas (*sealant*) agar celah tutup rapat dan kedap dari rembesan air hujan.
                    """)
                elif st.session_state.status_level == "Medium":
                    st.warning("""
                    **Medium ➔ Saran: Patching (Penambalan)**
                    
                    *Aksi Perbaikan:* Lakukan metode pengerukan atau penggalian lokal (*patching*) pada area aspal yang mengelupas atau mengalami retak kulit buaya, bersihkan sisa material lama, lalu isi dan padatkan kembali menggunakan material aspal hotmix baru.
                    """)
                elif st.session_state.status_level == "High":
                    st.error("""
                    **High ➔ Saran: Reconstruction (Pengaspalan Ulang / Rekonstruksi)**
                    
                    *Aksi Perbaikan:* Adanya kerusakan kategori berat berupa lubang struktural masif mewajibkan dilakukannya tindakan reconstruction struktur perkerasan jalan secara menyeluruh atau pembongkaran fondasi lokal agar daya dukung beban jalan kembali normal.
                    """)
            else:
                st.success("**Keterangan: Jalan Normal / Aman**")
...
# --- Halaman 2: Explore ---
    ...
    st.markdown("""
    <div style="background-color: #FF4B4B; padding: 15px; border-radius: 8px; margin-bottom: 15px;">
        <h2 style="text-align: center; color: white; margin: 0; font-family: sans-serif;">Jenis - Jenis Kerusakan Jalan</h2>
    </div>
    """, unsafe_allow_html=True)
    
    st.write("Berikut adalah penjabaran detail mengenai 6 jenis kerusakan aspal yang mampu diidentifikasi oleh sistem:")
    st.write("---")
    ...
    with k2:
        ...
        with st.container(border=True):
            st.markdown("### 7. Retak Pinggir (Edge Cracking)")
